...
export function PitwallShell() {
  ...
  const raceDuration = leaderCum.length
    ? leaderCum[leaderCum.length - 1]!
    : 0;
  ...
  const apiBoard = useMemo(() => {
    ...
    const finished =
      raceDuration > 0 && archiveElapsed >= raceDuration - 0.25;
...
-- AFTER INSERT: is_purple_s* / session_best`}</pre>
            </div>
            <div className="rounded-md bg-surface-2 p-3">
              <p className="mb-2 tracking-widest text-subtle uppercase">
                4. Derived trên UI
            ...
            {isOpenF1 && sampleLap && (
              <>
                <div className="rounded-md bg-surface-2 p-3">
                  ...
                  <pre className="whitespace-pre-wrap text-fg">{`driver ${sampleLap.driver_number} #${code}  L${sampleLap.lap_number}
s1 ${fmtSec(sampleLap.duration_sector_1)}  s2 ${fmtSec(sampleLap.duration_sector_2)}  s3 ${fmtSec(sampleLap.duration_sector_3)}
lap ${fmtLap(sampleLap.lap_duration)}`}</pre>
                ...
                >
                  <p className="mb-1 flex items-center gap-2 tracking-widest text-subtle uppercase">
                    AFTER INSERT trigger
                    {purpleFired && (
                      <span className="rounded bg-sector-purple/30 px-1.5 text-[10px] text-sector-purple">
                        FIRED
                      </span>
                    )}
                  </p>
                  <pre className="whitespace-pre-wrap text-fg">{`IF NEW.duration_s3 < MIN(s3) THEN is_purple_s3 = 1`}</pre>
                </div>
              </>
            )}
            {live && (
