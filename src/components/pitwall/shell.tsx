...
const PLAY_SPEEDS = [8, 16, 32, 64] as const;
...
export function PitwallShell() {
  ...
  const purpleLog = useMemo(() => {
    ...
    for (const l of rawLaps) {
      ...
      const items: [string, number | string | null | undefined][] = [
        ["S1", l.is_purple_s1 ? l.duration_sector_1 : null],
        ["S2", l.is_purple_s2 ? l.duration_sector_2 : null],
        ["S3", l.is_purple_s3 ? l.duration_sector_3 : null],
      ];
      for (const [kind, val] of items) {
...
function SidePanel({
  ...
  currentLap?: number;
  isOpenF1?: boolean;
  purpleLog?: { kind: string; lap: number; code: string; value: number }[];
}) {
          ...
          live ? (
            ...
            <div className="flex h-full min-h-0 flex-col overflow-y-auto">
              ...
              <section className="mx-3 mb-2 rounded-md bg-surface-2 px-3 py-2">
                <p className="mb-1.5 text-xs tracking-widest text-subtle uppercase">
                  INSERT INTO laps
                </p>
                {sampleLap ? (
                  <pre className="overflow-x-auto font-mono text-xs leading-relaxed text-fg">{`driver_number: ${sampleLap.driver_number}   #${code}
...
lap_duration:  ${fmtLap(sampleLap.lap_duration)}`}</pre>
                  ...
                  <p className="text-sm text-muted">Waiting for lap…</p>
                )}
              </section>

              <section
                className={`mx-3 mb-2 rounded-md px-3 py-2 ${
                  purpleFired ? "bg-sector-purple/15 ring-1 ring-sector-purple/40" : "bg-surface-2"
                }`}
              >
                <div className="mb-1.5 flex items-center gap-2">
                  ...
                  {purpleFired && (
                    <span className="rounded bg-sector-purple px-1.5 py-0.5 text-[10px] font-bold tracking-wider text-white uppercase">
                      FIRED
                    </span>
                  )}
                </div>
                <pre className="font-mono text-xs leading-relaxed text-fg whitespace-pre-wrap">{`AFTER INSERT ON laps FOR EACH ROW
IF NEW.duration_s3
  < (SELECT MIN(duration_s3)
     FROM laps WHERE session_key = NEW.session_key)
THEN SET NEW.is_purple_s3 = 1;`}</pre>
                {purpleFired && sampleLap && (
                  <p className="mt-2 font-mono text-xs text-sector-purple">
                    {[
                      sampleLap.is_purple_s1 && `S1: ${fmtSec(sampleLap.duration_sector_1)}`,
                      sampleLap.is_purple_s2 && `S2: ${fmtSec(sampleLap.duration_sector_2)}`,
                      sampleLap.is_purple_s3 && `S3: ${fmtSec(sampleLap.duration_sector_3)}`,
                    ]
                      .filter(Boolean)
                      .join("  ·  ")}
                  </p>
                ...
                <ul className="space-y-1 font-mono text-xs text-fg">
                  <li>
                    ...
                    <span className="ml-2 text-subtle">
                      RANK() OVER (ORDER BY cumulative_time)
                    </span>
                  </li>
                  <li>
                    gap_to_leader ={" "}
                    {row?.gap_to_leader == null
                      ? "Leader"
                      : `+${Number(row.gap_to_leader).toFixed(3)}`}
                    <span className="ml-2 text-subtle">
                      this.cumulative − leader.cumulative
                    </span>
                  </li>
                  <li>
                    gap_to_ahead ={" "}
                    {row?.gap_to_car_ahead == null
                      ? "—"
                      : `+${Number(row.gap_to_car_ahead).toFixed(3)}`}
                  </li>
                  <li>
                    is_purple ={" "}
                    {purpleFired
              ...
              <div className="min-h-0 flex-1">
                ...
                {purpleLog.length === 0 ? (
                  <p className="px-3 text-sm text-muted">No session-best yet — Play to advance.</p>
                ) : (
                  <ol>
                    {purpleLog.map((e, i) => (
                      <li
                        key={`${e.kind}-${e.lap}-${e.code}-${i}`}
                        className="flex items-center gap-2 border-t border-border px-3 py-1.5"
                      >
                        <span className="w-8 font-mono text-xs text-sector-purple">
                          {e.kind}
                        </span>
