...
export function TimingHeader() {
  ...
  const isLiveBaku = sessionKey === 11377;
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const year = isLiveBaku || sessionKey >= 11300 ? 2026 : 2024;
        ...
        if (hit) {
          setOpenMeta({
            ...
            country_name: hit.country_name
              ...
              : undefined,
            year: hit.year != null ? Number(hit.year) : year,
          });
          return;
        }
      } catch {}
  ...
  }, [sessionKey, isOpenF1, isLiveBaku]);

  const weather = isOpenF1 ? archiveWeather : simWeather;
  const showWeatherBar =
    view === "timing" &&
    weather &&
    (isOpenF1 ? archiveMaxLap > 0 || isLiveBaku : lap != null);
        ...
        <div className="flex shrink-0 flex-col items-end gap-1.5">
          <div className="flex items-center gap-1">
            <Button
              size="sm"
              variant={
                view === "timing" && (live || isLiveBaku) ? "secondary" : "ghost"
              }
              onClick={() => goLive()}
              className="gap-1.5"
              aria-label="Live"
            ...
            <Button
              size="sm"
              variant={
                view === "standings" || (view === "timing" && !live && !isLiveBaku)
                  ? "secondary"
                  : "ghost"
              }
              onClick={() => openArchive()}
              ...
              >
                <span
                  className={cn(
                    ...
                    (isLiveBaku || ((live || replay) && playing && !finished)) &&
                      "live-dot",
                  )}
                />
                {isLiveBaku
        ...
        <div className="flex flex-wrap items-center gap-x-4 gap-y-2 rounded-lg bg-surface px-3 py-2 shadow-[var(--shadow-border)]">
          ...
          <span className="ml-auto flex items-baseline gap-2">
            ...
            <span className="font-mono text-lg font-semibold tabular">
              {String(isOpenF1 ? archiveLap : (lap ?? 0)).padStart(2, "0")}
